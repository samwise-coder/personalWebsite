...
// cross(To);
function initChart(yData) {
  const gantt =document.getElementById('gantt')
  const ganttChart = echarts.init(gantt)
  const option = {
    grid:{
      show:false,
      right:20,
    ...
    series:[{
      type:'bar',
      stack:'duration',
      data:['2024-1-1','2024-2-3','2024-4-6','2024-4-20','2024-4-30','2024-5-6','2024-5-16']
    },
    {
      type:'bar',
      stack:'duration',
      data:['2023-1-1','2023-2-3','2023-4-6','2023-4-20','2023-4-30','2023-5-6','2023-5-16']
    },]
  }
...
function fitness(mpc, ppc,xJobs) {
  ...
  ppc.forEach((ele,index)=>{
    ...
    if(Mijh[machineKey].length===0){// 在机器mi是第一道加工工序
      if(h===1){// 是工件的第一道工序
        _res._St=0
        _res._Et=duration
      ...
      Mijh[machineKey].push({process:`O${j}${h}`,St:_res._St,Et:_res._Et})
    }else{ // 否则找到机器mi上所有间隔空闲时间段[TSi,TEi],注：当前工序前一道工序Oj(h-1)的结束时间后的才是有效区间
      let idlePeriod=[]
      // 从第一个工序前的空白开始
      ...
      if(firstSparePeriod.length){
        Mijh[machineKey].splice(firstSparePeriod[0].fspdex,0,{process:`O${j}${h}`,St:ta,Et:ta+duration})
        _res._St=ta
        _res._Et=ta+duration
      }else{// 如果没有可插入空间则安排到当前机器最后一道工序的结束时间
        ...
        Mijh[machineKey].push({process:`O${j}${h}`,St:tb,Et:tb+duration})
        _res._St=tb
        _res._Et=tb+duration
      }
    }

    Ojh[`O${j}${h}`]={machine:machineKey,duration:duration,St:_res._St,Et:_res._Et}
...
function machineGlobalSelection(xJobs,xMachines) {
  ...
  selectionQueue.forEach(ele => {
    const gsiItem = selectSingleProcess(xJobs[ele],xMachineTimeList,xMachines);
